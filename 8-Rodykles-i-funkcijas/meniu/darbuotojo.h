...
public:
    ...
    void rodyti() const override {
        ...
        std::cout << "1. " << (!darbuotojas_.gautiVarda().empty() ? "Keisti" : "Įrašyti") << " vardą: "
                  << darbuotojas_.gautiVarda()
                  << std::endl;
        ...
        std::cout << "4. " << (darbuotojas_.gautiAmziu() ? "Keisti" : "Įrašyti") << " amžių: "
                  << darbuotojas_.gautiAmziu()
                  << std::endl;
        std::cout << "5. " << (darbuotojas_.gautiPatirti() ? "Keisti" : "Įrašyti") << " patirtį: "
                  << darbuotojas_.gautiPatirti()
                  << std::endl;
...
protected:
    ...
    void apkeistiPasirinkimoTvarka(unsigned kaNr) {
        unsigned suKuoNr;
...
public:
    ...
    void rodyti() const override {
        ...
        for (unsigned i = 0; i < Darbuotojas::DUOMENU_KIEKIS; ++i) {
            std::cout << i + 1 << ". " << Darbuotojas::DuomenuPasirinkimoPavadinimas[spausdinimoTvarka_[i]]
                      << std::endl;
        }
    }
